...
class Config_Train(Config_runscript_base):
    ...
    allowed_datasets = known_datasets
    ...
    def run_tfrecords(self):
        ...
        """
        method_name = Config_Train.run.__name__
        ...
        self.source_dir = Config_Train.keyboard_interaction(expdir_req_str, Config_Train.check_expdir_2,
        ...
        if ntf_dirs == 1:
            ...
            self.source_dir = Config_Train.keyboard_interaction(seq_req_str, Config_Train.check_expdir_2,
                                                                seq_err, ntries=2,
    ...
    def list_dirs(self):
        """Quick hack to negate reliance on tf-records."""
        return [path.name for path in Path(self.source_dir).iterdir() if path.is_dir()]
    ...
    # -----------------------------------------------------------------------------------
    #
    def check_expdir_2(exp_dir, silent=False):
        """Quick hack to negate reliance on tf-records."""
        return True
